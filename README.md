...
│       ├── pages/        # 页面（7 个）
│       ├── store/        # Zustand 状态管理（含 E2EE 密钥内存态）
│       ├── styles/       # CSS 设计令牌 + 组件样式
│       ├── utils/        # 工具函数（E2EE WebCrypto 加解密）
...
- **双因素认证**：TOTP 2FA 支持
- **端到端加密**：可选 E2EE，浏览器内加解密（PBKDF2 + AES-256-GCM），服务器只存密文
- **HTTPS/TLS**：支持正式证书或自动生成自签证书
- **存储后端**：本地磁盘 / S3 兼容对象存储
- **多种浏览**：列表 / 大图标网格 / 时间线
